...
- Interactive Demo: [https://ct-query-to-visualization-agent.vercel.app/demo](https://ct-query-to-visualization-agent.vercel.app/demo)
- API Documentation (Swagger): [https://ct-query-to-visualization-agent.vercel.app/docs](https://ct-query-to-visualization-agent.vercel.app/docs)
...
## Example Runs
The following examples were generated by the deployed service using live ClinicalTrials.gov data. Full JSON responses are included in the
...
```
curl -s -X POST \
  "https://ct-query-to-visualization-agent.vercel.app/api/v1/visualizations" \
  -H "Content-Type: application/json" \
  -d '{
    "query": "How have pembrolizumab trials changed since 2015?",
    "max_studies": 200,
    "include_citations": true,
    "max_citations_per_datum": 5
  }' | python -m json.tool > examples/time-trend.json
```
#### Actual JSON output
examples/time-trend.json⁠
...
curl -s -X POST \
  ...
  -H "Content-Type: application/json" \
  -d '{
    "query": "Show the distribution of trial phases for pembrolizumab.",
    "max_studies": 200,
    "include_citations": true,
    "max_citations_per_datum": 5
  }' | python -m json.tool > examples/phase-distribution.json
```
#### Actual JSON output
examples/phase-distribution.json⁠￼
